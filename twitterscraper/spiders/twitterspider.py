...
class HashtagSpider(scrapy.Spider):
    ...
    def __init__(self, filename=""):
        if not filename:
            sys.exit(
                "Please provide the input filename also. Example \n\n$ python3 hashtags.py myinput.csv"
            )
    ...
    def start_requests(self):
        ...
        if len(hashtags) == 0:
            sys.exit(
                "Emplty File detected.Please provide hashtags separated by newlines"
            )
        else:
        ...
        for hashtag in hashtags:
            if hashtag:
                ...
                yield scrapy.Request(
                    search_url, callback=self.find_tweets, dont_filter=True
                )
    ...
    def parse_tweet(self, response):
        ...
        date_time = parser.parse(date_time.replace("-", "")).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
